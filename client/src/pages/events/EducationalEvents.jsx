// src/pages/events/EducationalEvents.jsx
...
export default function EducationalEvents() {
  ...
  useEffect(() => {
    (async () => {
      const events = await api.listEvents("educational");
      setItems(events);
    })();
  }, []);
  ...
  const handleBook = (eventId) => {
    ...
    if (!token) {
      ...
      alert(`✅ Booking successful for educational event: ${eventId}`);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50 bg-fixed">
      ...
      <section className="pt-24 max-w-7xl mx-auto px-6">
        <header className="mb-6">
          <h1 className="text-3xl font-bold text-purple-800">Educational & School Events</h1>
          <p className="text-gray-600">School and educational functions made memorable.</p>
        </header>
