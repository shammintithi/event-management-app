// src/pages/events/CorporateEvents.jsx
...
export default function CorporateEvents() {
  ...
  useEffect(() => {
    (async () => {
      const events = await api.listEvents("corporate");
      setItems(events);
    })();
  }, []);
  ...
  const handleBook = (eventId) => {
    ...
    if (!token) {
      ...
      // Later replace with backend booking API
      alert(`✅ Booking successful for corporate event: ${eventId}`);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50 bg-fixed">
      ...
      <section className="pt-24 max-w-7xl mx-auto px-6">
        <header className="mb-6">
          <h1 className="text-3xl font-bold text-purple-800">Corporate Events</h1>
          <p className="text-gray-600">Professional gatherings with elegant planning.</p>
        </header>
