// src/pages/events/ReligiousEvents.jsx
import { useEffect, useState } from "react";
import { api } from "../../utils/api";
import EventCard from "../../components/EventCard";
import { Link, useNavigate } from "react-router-dom";

export default function ReligiousEvents() {
  const [items, setItems] = useState(null);
  const navigate = useNavigate();

  useEffect(() => {
    (async () => {
      const events = await api.listEvents("religious");
      setItems(events);
    })();
  }, []);
  ...
  const handleBook = (eventId) => {
    const token = localStorage.getItem("token"); // token will be saved at login

    if (!token) {
      // If not logged in → go to login page
      navigate("/login");
    } else {
      // If logged in → you can later call backend booking API
      alert(`✅ Booking successful for event: ${eventId}`);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50 bg-fixed">
      ...
      <section className="pt-24 max-w-7xl mx-auto px-6">
        <header className="mb-6">
          <h1 className="text-3xl font-bold text-purple-800">Religious Events</h1>
          <p className="text-gray-600">Spiritual gatherings with precision planning.</p>
        </header>
