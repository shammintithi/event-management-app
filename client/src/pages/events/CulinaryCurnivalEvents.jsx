// src/pages/events/CulinaryEvents.jsx
import { useEffect, useState } from "react";
import { api } from "../../utils/api";
import EventCard from "../../components/EventCard";
import { Link, useNavigate } from "react-router-dom";

export default function CulinaryEvents() {
  const [items, setItems] = useState(null);
  const navigate = useNavigate();

  useEffect(() => {
    (async () => {
      const events = await api.listEvents("culinary");
      setItems(events);
    })();
  }, []);
  ...
  const handleBook = (eventId) => {
    ...
    if (!token) {
      ...
      alert(`✅ Booking successful for culinary event: ${eventId}`);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-indigo-50 bg-fixed">
      ...
      <section className="pt-24 max-w-7xl mx-auto px-6">
        <header className="mb-6">
          <h1 className="text-3xl font-bold text-purple-800">Culinary Events</h1>
          <p className="text-gray-600">Taste the world with unforgettable food gatherings.</p>
        </header>
