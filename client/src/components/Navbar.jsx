...
const navLinkClass =
  ({ isActive }) =>
    "block px-4 py-2 hover:text-indigo-600 " +
...
export default function Navbar() {
  ...
  const events = [
    ...
    { label: "Culinary Curnival", to: "/events/culinary-curnival" }, // spelling per request
  ];

  return (
    <header className="sticky top-0 bg-white/70 backdrop-blur supports-[backdrop-filter]:bg-white/60 z-50 border-b">
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            ...
            >
              ...
              {eventsOpen && (
                <div className="absolute left-0 mt-2 w-72 rounded-lg border bg-white dropdown-shadow">
                  <ul className="py-2">
                    {events.map((e) => (
                      <li key={e.to}>
                        <NavLink to={e.to} className={({isActive}) =>
                          "flex px-4 py-2 hover:bg-gray-50 " + (isActive ? "text-indigo-600" : "text-gray-700")
                        }>
                          {e.label}
                        </NavLink>
                      </li>
          ...
          <div className="md:hidden border-t py-2">
            ...
            <details className="px-2">
              ...
              <ul className="ml-2">
                {events.map((e) => (
                  <li key={e.to}>
                    <NavLink to={e.to} className={navLinkClass} onClick={() => setMobileOpen(false)}>
                      {e.label}
                    </NavLink>
                  </li>
                ))}
